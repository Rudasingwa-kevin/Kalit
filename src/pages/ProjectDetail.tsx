...
const budgetLineColors = ['#2563EB', '#1F2937', '#22C55E', '#F59E0B', '#8B5CF6']
        ...
        >
                      ...
                      {project.milestones?.map((ms: any, i: number) => (
                        <motion.div key={ms.id} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="relative flex items-start gap-3 md:gap-4 pl-1">
                          ...
                          <div className="flex-1 pb-2">
                            ...
                            <p className="text-[10px] md:text-xs text-gray-400 mt-0.5">{new Date(ms.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                          </div>
                    ...
                    {project.tasks?.slice(0, 5).map((task: any) => (
                      <div key={task.id} className="flex items-start gap-2.5 md:gap-3 p-2.5 md:p-3 rounded-[10px] md:rounded-[12px] hover:bg-surface-hover transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[9px] md:text-[11px] text-gray-400 mt-0.5">{task.assignee?.name ?? 'Unassigned'} · {task.date ? new Date(task.date).toLocaleDateString() : ''}</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'Budget' && (
            <div className="bg-white rounded-[20px] border border-border/50 p-4 md:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
                {project.budgetLines?.map((line: any, i: number) => (
                  <div key={line.id}>
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: budgetLineColors[i % budgetLineColors.length] }} />
                      <span className="text-[10px] md:text-xs font-medium text-gray-500">{line.category}</span>
                    </div>
                    <p className="text-xs md:text-sm font-bold text-primary">{formatCurrency(line.spent)}</p>
                    <p className="text-[10px] md:text-[11px] text-gray-400">of {formatCurrency(line.allocated)}</p>
                    <div className="w-full h-1.5 rounded-full bg-border/50 mt-2 overflow-hidden">
                      <motion.div className="h-full rounded-full" style={{ backgroundColor: budgetLineColors[i % budgetLineColors.length] }} initial={{ width: 0 }} whileInView={{ width: `${(line.spent / line.allocated) * 100}%` }} viewport={{ once: true }} transition={{ duration: 1 }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2">
                {project.tasks?.map((task: any) => (
                  <div key={task.id} className="flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-[12px] md:rounded-[14px] hover:bg-surface-hover transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs md:text-sm font-medium ${task.done ? 'text-gray-400 line-through' : 'text-primary'}`}>{task.title}</p>
                      <p className="text-[9px] md:text-[11px] text-gray-400 mt-0.5">{task.date ? new Date(task.date).toLocaleDateString() : ''}</p>
                    </div>
                    <span className="text-[10px] md:text-xs text-gray-400 font-medium hidden sm:inline">{task.assignee?.name ?? 'Unassigned'}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'Materials' && (
            <div className="space-y-3 md:space-y-4">
              {project.materials?.map((mat: any, i: number) => {
                const usagePercent = mat.used + mat.remaining > 0 ? (mat.used / (mat.used + mat.remaining)) * 100 : 0
                const trendData = Array.isArray(mat.trend) ? mat.trend : []
                return (
                  <motion.div key={mat.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="bg-white rounded-[14px] md:rounded-[16px] border border-border/50 p-4 md:p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h4 className="text-sm md:text-base font-bold text-primary">{mat.inventoryItem?.name ?? 'Material'}</h4>
                        <p className="text-[10px] md:text-xs text-gray-400">{mat.used} {mat.inventoryItem?.unit ?? ''} used · {mat.remaining} remaining</p>
                      </div>
                    ...
                    <div className="flex items-end gap-[3px] h-8 md:h-10">
                      {trendData.map((val: number, j: number) => (
                        <motion.div key={j} className="flex-1 rounded-sm bg-accent/20" initial={{ height: 0 }} whileInView={{ height: `${(val / Math.max(...trendData)) * 100}%` }} viewport={{ once: true }} transition={{ duration: 0.5, delay: j * 0.05 }} />
                      ))}
                    </div>
                  </motion.div>
                )
            ...
            <div className="bg-white rounded-[20px] border border-border/50 p-4 md:p-6">
              ...
              <div className="space-y-2 md:space-y-3">
                {project.workers?.map((w: any, i: number) => (
                  <motion.div key={w.id} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }} className="flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-[12px] md:rounded-[14px] hover:bg-surface-hover transition-colors">
                    <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-primary/8 flex items-center justify-center flex-shrink-0">
                      <span className="text-[10px] md:text-xs font-bold text-primary">{w.user?.name?.split(' ').map((n: string) => n[0]).join('') ?? '?'}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs md:text-sm font-semibold text-primary truncate">{w.user?.name ?? 'Unknown'}</h4>
                      <p className="text-[10px] md:text-xs text-gray-400 truncate">{w.user?.email ?? ''}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="text-xs md:text-sm font-bold text-primary">{w.hours}h</p>
          ...
          {activeTab === 'Activity' && (
            <div className="bg-white rounded-[20px] border border-border/50 p-4 md:p-6">
              ...
              <div className="space-y-3 md:space-y-4">
                {project.activities?.map((log: any, i: number) => (
                  <motion.div key={log.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="flex items-start gap-3 md:gap-4 p-3 md:p-4 rounded-[12px] md:rounded-[14px] hover:bg-surface-hover transition-colors">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-1.5 md:gap-2 mt-1">
                        <span className="text-[10px] md:text-xs text-gray-400">{log.user?.name ?? 'System'}</span>
                        <span className="text-[10px] md:text-xs text-gray-300">·</span>
                        <span className="text-[10px] md:text-xs text-gray-400">{timeAgo(log.timestamp)}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
