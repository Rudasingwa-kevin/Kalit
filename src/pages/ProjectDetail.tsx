...
export default function ProjectDetail() {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="bg-white rounded-[20px] border border-border/50 p-8">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div className="flex items-start gap-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  <span className="flex items-center gap-1.5 text-xs text-gray-400"><MapPin className="w-3.5 h-3.5" />{project.location}</span>
                  <span className="flex items-center gap-1.5 text-xs text-gray-400"><Users className="w-3.5 h-3.5" />{project.engineer}</span>
            ...
            <div className="flex items-center gap-3">
              <button className="flex items-center gap-2 px-4 py-2.5 border border-border rounded-[12px] text-sm font-semibold text-primary hover:bg-surface-hover transition-colors">
                <Share2 className="w-4 h-4" /> Share
              </button>
              <button className="flex items-center gap-2 px-5 py-2.5 bg-accent text-white rounded-[12px] text-sm font-semibold hover:bg-accent-dark transition-colors">
      ...
      <FadeInUp delay={0.1}>
        <div className="bg-white rounded-[16px] border border-border/50 p-1.5 inline-flex gap-1">
          {tabs.map(tab => (
            <button
              key={tab}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {activeTab === 'Overview' && (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-[20px] border border-border/50 p-6">
                  ...
                  <div className="relative">
                    ...
                    <div className="space-y-5">
                      {milestones.map((ms, i) => (
                        <motion.div key={ms.id} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="relative flex items-start gap-4 pl-1">
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center z-10 flex-shrink-0 ${ms.status === 'completed' ? 'bg-success text-white' : ms.status === 'in_progress' ? 'bg-accent text-white' : 'bg-white border-2 border-border text-gray-300'}`}>
                            {ms.status === 'completed' ? <CheckCircle2 className="w-4 h-4" /> : ms.status === 'in_progress' ? <Target className="w-3.5 h-3.5" /> : <Circle className="w-2 h-2" />}
                          </div>
                          <div className="flex-1 pb-2">
                            <h4 className={`text-sm font-semibold ${ms.status === 'upcoming' ? 'text-gray-400' : 'text-primary'}`}>{ms.name}</h4>
                            <p className="text-xs text-gray-400 mt-0.5">{ms.date}</p>
                          </div>
                          {ms.status === 'in_progress' && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-accent/10 text-accent">Active</span>}
                        </motion.div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-[20px] border border-border/50 p-6">
                  <h3 className="text-base font-bold text-primary mb-4">Recent Tasks</h3>
                  <div className="space-y-3">
                    {recentTasks.map(task => (
                      <div key={task.id} className="flex items-start gap-3 p-3 rounded-[12px] hover:bg-surface-hover transition-colors">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${task.done ? 'bg-success text-white' : 'border-2 border-border'}`}>
                ...
                <div className="bg-white rounded-[20px] border border-border/50 p-6">
                  ...
                  <div className="space-y-2">
                    {['Log Expense', 'Add Material', 'Upload Photo'].map(label => (
                      <button key={label} className="w-full flex items-center gap-3 p-3 rounded-[12px] text-left hover:bg-surface-hover transition-colors">
          ...
          {activeTab === 'Budget' && (
            <div className="bg-white rounded-[20px] border border-border/50 p-6">
              <h3 className="text-lg font-bold text-primary mb-6">Budget Breakdown</h3>
          ...
          {activeTab === 'Tasks' && (
            <div className="bg-white rounded-[20px] border border-border/50 p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <button className="flex items-center gap-2 px-4 py-2.5 bg-accent text-white rounded-[12px] text-sm font-semibold">
              ...
              <div className="space-y-2">
                {recentTasks.map(task => (
                  <div key={task.id} className="flex items-center gap-4 p-4 rounded-[14px] hover:bg-surface-hover transition-colors group">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${task.done ? 'bg-success text-white' : 'border-2 border-border group-hover:border-accent'}`}>
          ...
          {activeTab === 'Materials' && (
            <div className="space-y-4">
              {projectMaterials.map((mat, i) => {
                ...
                return (
                  <motion.div key={mat.name} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="bg-white rounded-[16px] border border-border/50 p-6">
          ...
          {activeTab === 'Workers' && (
            <div className="bg-white rounded-[20px] border border-border/50 p-6">
              <h3 className="text-lg font-bold text-primary mb-6">Team Members</h3>
              <div className="space-y-3">
                {workers.map((worker, i) => (
                  <motion.div key={worker.name} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }} className="flex items-center gap-4 p-4 rounded-[14px] hover:bg-surface-hover transition-colors">
                    <div className="w-10 h-10 rounded-full bg-primary/8 flex items-center justify-center flex-shrink-0">
                      <span className="text-xs font-bold text-primary">{worker.name.split(' ').map(n => n[0]).join('')}</span>
                    </div>
                    <div className="flex-1 min-w-0">
            ...
            <div className="bg-white rounded-[20px] border border-border/50 p-6">
              ...
              <div className="space-y-4">
                {activityLog.map((log, i) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="flex items-start gap-4 p-4 rounded-[14px] hover:bg-surface-hover transition-colors">
